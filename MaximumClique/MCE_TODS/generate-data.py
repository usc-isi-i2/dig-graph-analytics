...
def f(x):
    ...
    for object in line:
        if 'seller_uri' in object:
            print object['seller_uri'],type(object['seller_uri']),hash(object['seller_uri'])
        if 'phones' in object:
            for phone in object['phones']:
                print phone,type(phone),hash(phone)
        if 'emails' in object:
            for email in object['emails']:
                print email,type(email),hash(email)
...
if __name__ == '__main__':
    ...
    intermediate_rdd.saveAsTextFile("intermediate")